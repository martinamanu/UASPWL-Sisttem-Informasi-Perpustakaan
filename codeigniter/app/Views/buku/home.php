            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <?php if ($auth->getUserPriv($user["token"])->can_add_book) : ?>
                        <a class="btn btn-success mr-2 text-white float-right" href="/buku/tambah">Tambah Buku</a>
                    <?php endif ?>
                </div>
                                    ...
                                    <?php if ($auth->getUserPriv($user["token"])->can_edit_book && $auth->getUserPriv($user["token"])->can_delete_book) : ?>
                                        <td class="text-center">
                                            <?php if ($auth->getUserPriv($user["token"])->can_edit_book) : ?>
                                                <a class="btn btn-primary mr-2 text-white" href="/buku/edit/<?= $row['id']; ?>">Edit</a>
                                            <?php endif ?>
                                            <?php if ($auth->getUserPriv($user["token"])->can_delete_book) : ?>
                                                <a class="btn btn-danger mr-2 text-white" href="/buku/hapus/<?= $row['id']; ?>">Hapus</a>
                                            <?php endif ?>
                                        </td>
                                    <?php endif ?>
